...
(function () {
  ...
  var css = [
    ...
    '@media (max-width:767px){.cedar-hero-watch:not(.cedar-card-watch){right:10px;}}',   /* v1.88.2 (client): phones — the standalone pill sits 10px off the right edge (desktop keeps 22px) */
    ...
    '.cedar-foot-bottom{display:flex;justify-content:space-between;align-items:flex-end;margin-top:20px;}',   /* 20px below the big lockup. v1.89.1: flex-end (was center) so the copyright and "Built by Origin" BOTTOM-align even when one box carries extra padding */
    ...
    '.cedar-foot-word{flex:0 0 79.5%;aspect-ratio:641/70;-webkit-mask-size:99% auto!important;mask-size:99% auto!important;}',   /* v1.89.1 (client): the wordmark asset is cropped flush, so contain painted the C\'s curve at the box\'s exact left edge and rendering shaved it (visible on phones). 99% centered = ~2px breathing room each side; the size change is invisible. */
    '@media (max-width:767px){.site-footer{padding:0!important;}.cedar-foot{padding:10px!important;}.cedar-foot-cols{flex-direction:column;min-height:0;}.cedar-foot-col.c1{flex:0 0 auto;padding:12px 0 0;}.cedar-foot-col.c2{border-left:0;padding:24px 0 0;}.cedar-foot-bottom{padding-top:20px;}a.cedar-foot-lockup{margin-top:36px;}}',   /* v1.89.1: the extra mobile spacing moved off .cf-copy onto the row itself — padding on just the copyright made its flex box taller and threw "Built by Origin" off its baseline */
  ...
  onReady(function () {
    ...
    /* v1.87.2 (client): a film's poster comes from the FILM — each video asset fetches its own Vimeo
       thumbnail (oEmbed, hash-aware for unlisted links) and uses that in the coverflow; the card's CMS
       image is only the fallback if Vimeo doesn't answer. Why: duplicated gallery items kept the copied
       item's still in the Image field, and the page never exposes it (the muted loop covers it) — the
       coverflow was the one place the stale image surfaced. Now it self-corrects. */
    assets.forEach(function (as, i) {
      if (as.type !== 'video' || !as.id || !window.fetch) return;
      try {
        fetch('https://vimeo.com/api/oembed.json?width=1280&url=' + encodeURIComponent('https://vimeo.com/' + as.id + (as.hash ? '/' + as.hash : '')))
          .then(function (r) { return r.ok ? r.json() : null; })
          .then(function (d) {
            if (!d || !d.thumbnail_url) return;
            as.src = d.thumbnail_url;
            var card = gvCards[i];                       /* coverflow already built (opened before the fetch landed)? update it live */
            if (!card || !card._media) return;
            var im = card._media.querySelector('img');
            if (!im) { im = document.createElement('img'); im.alt = ''; im.draggable = false; card._media.insertBefore(im, card._media.firstChild); }
            im.src = d.thumbnail_url;
          }).catch(function () {});
      } catch (e) {}
    });
